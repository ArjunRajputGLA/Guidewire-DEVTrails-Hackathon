...
**Inspiration**  
The insurance and worker's compensation industry often struggles with slow, manual claims processing, delayed payouts, and undetected fraud. Inspired by the robust capabilities of core systems like Guidewire, we wanted to build a modern, AI-driven platform that automates the lifecycle of a claim. Our goal was to create a seamless experience combining real-time notifications, intelligent fraud detection, and intuitive role-based dashboards for both administrators and workers/policyholders.

**How we built your project**  
We built the platform using a modern full-stack architecture:
*   **Frontend:** We utilized Next.js with the App Router to build distinct, role-based dashboards (`/admin` and `/worker`). The UI is strictly typed using TypeScript for maintainability.
*   **Backend & Database:** We leveraged Supabase (PostgreSQL) as our core database. We wrote custom SQL scripts to handle complex logic directly at the database layer, including automated notification triggers and real-time updates.
*   **AI Integration:** To automate risk assessment, we integrated Google Gemini. When a worker files a new claim, the AI analyzes the claim patterns and text context. We calculate a dynamic risk score for claims using a weighted mathematical model: 
    $$ Risk = \alpha \left( \frac{Claim Amount}{Max Coverage} \right) + \beta \left( \mathcal{P}(Fraud | ML_{context}) \right) $$
    where $\mathcal{P}$ represents the probability of fraud generated by the Gemini model. Based on the threshold, a database trigger automatically flags the claim for administrative review.
*   **Authentication & State:** We used Supabase Auth combined with custom React context to securely separate administrative actions from worker actions like weekly payout tracking.

**Challenges we faced**  
*   **Real-Time Subscriptions:** Ensuring that administrators get immediate notification bells when a worker submits a claim required fine-tuning Supabase's real-time channels and PostgreSQL Row Level Security (RLS) policies.
*   **AI Context and Prompting:** Calibrating the Gemini model to accurately detect fraudulent anomalies without generating a high volume of false positives was complex. We had to iteratively test our AI models to get reliable confidence scores.
*   **Schema Design:** Mapping the complex relationships between Workers, Policies, Claims, and Payouts required extensive SQL planning to maintain data integrity while ensuring fast queries.

**What we learned**  
*   We deepened our understanding of the Next.js App Router and server-side components.
*   We mastered advanced PostgreSQL features, specifically writing database triggers and functions to offload business logic from the Node.js backend directly to the data layer.
*   We learned how to orchestrate LLMs (Google Gemini) within a transactional flow to augment decision-making in a high-stakes domain like insurance.

---

## 🛠️ Built With

Next.js, React, TypeScript, Node.js, Supabase, PostgreSQL, Google Gemini, Tailwind CSS, AWS Lambda, Vercel
