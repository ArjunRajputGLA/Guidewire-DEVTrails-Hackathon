...
// -------------------------------------------------------
router.post('/calculate-premium', async (req: Request, res: Response) => {
  const {
    avg_daily_earnings,
    working_days,
    city_zone,
  ...
  if (weekly < 3500) {
    tier = 'starter'; basePremium = 29; maxPayout = 1500
  } else if (weekly >= 5500) {
    tier = 'pro'; basePremium = 79; maxPayout = 4000
  }
...
router.post('/complete', async (req: Request, res: Response) => {
  ...
  if (!profile.data) missing.push('profile')
  if (!kyc.data)     missing.push('kyc_documents')
  if (!gig.data)     missing.push('gig_profile')
  if (!income.data)  missing.push('income_data')
  if (!payment.data) missing.push('payment_info')
